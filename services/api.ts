...
const localService = {
    ...
    saveItem: async (item: Omit<Item, 'id' | 'createdAt'>): Promise<Item> => {
        ...
        localStorage.setItem(STORAGE_KEY_ITEMS, JSON.stringify([newItem, ...items]));
        return newItem;
    },
    updateItem: async (id: string, updates: Partial<Omit<Item, 'id' | 'createdAt'>>): Promise<Item> => {
        const items = await localService.getAllItems();
...
export const itemsService = {
    ...
    create: async (item: Omit<Item, 'id' | 'createdAt'>) => {
        if (useRemote) {
            try { return await remoteService.saveItem(item); } catch (e) { console.warn("Fallo Supabase save", e); }
        }
        return await localService.saveItem(item);
    },
    update: async (id: string, updates: Partial<Omit<Item, 'id' | 'createdAt'>>) => {
        if (useRemote) {
             try { return await remoteService.updateItem(id, updates); } catch (e) { console.warn("Fallo Supabase update", e); }
        }
        return await localService.updateItem(id, updates);
    },
    delete: async (id: string) => {
        if (useRemote) {
             try { return await remoteService.deleteItem(id); } catch (e) { console.warn("Fallo Supabase delete", e); }
        }
...
export const categoriesService = {
    getAll: async () => {
        ...
        } catch (e) { console.warn("Fallo Supabase categories", e); }
        return await localService.getAllCategories();
    },
    create: async (name: string) => {
        if (useRemote) {
             try { return await remoteService.createCategory(name); } catch (e) { console.warn("Fallo Supabase create category", e); }
        }
